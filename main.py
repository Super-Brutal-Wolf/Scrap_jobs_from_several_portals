...
def search_ziprecruiter(preprocessed_title: str, job_region: str, job_country: str) -> list:
    job_title = preprocessed_title
    jobs_list = []
    try:
        location = job_country if job_region == "Unknown" else job_region
        logger.info(f"Searching for Ziprecruiter for {job_title} in {location}")

        searching_title = job_title.strip().replace(" ", "+")
        searching_location = location.strip().replace(" ", "+")
        searching_url = f"https://www.ziprecruiter.com/jobs-search?search={searching_title}&location={searching_location}&radius=25&days=1"
        ...
        logger.info(f"Found {len(dataset_items)} jobs from Ziprecruiter")
    
        for job in dataset_items:
            if job.get("Title") is None:
                continue
            pre_company_url = "https://www.ziprecruiter.com/" + job.get("OrgURL")

            job_data = {
                "company": job.get("OrgName", "Not define"),
                "company_url": pre_company_url,
                "salary": job.get("FormattedSalary"),
                "linkedin_url": "Not define", 
                "indeed_url": job.get("JobURL", "Not define"), 
                "apply_url": job.get("intentToApplyUrl", "Not define"), 
                "job_url": job.get("JobURL", "Not define"), 
                "contract_type": job.get("EmploymentType"), 
                "job_description": job.get("description", "Not define"), 
                "experience_level": "Not define", 
                "location": job.get("jobDetails").get("model").get("getAutoBanners").get("job_location"), 
                "posted_time": job.get("humanDate"), 
                "published_at": job.get("humanDate"), 
        ...
        error_message = f"Error searching Ziprecruiter: {str(e)}"
    ...
    try:
        ...
        if "," in data.jobTitle:
            ...
            for title in updated_job_title:
                ...
                time.sleep(10)
                ziprecruiter_jobs = search_ziprecruiter(preprocessed_title, data.region, data.country)
                jobs_list.extend(ziprecruiter_jobs)
                time.sleep(10)
        else:
            ...
            jobs_list.extend(glassdoor_jobs)
            time.sleep(10)
            ziprecruiter_jobs = search_ziprecruiter(data.jobTitle, data.region, data.country)
            jobs_list.extend(ziprecruiter_jobs)
            time.sleep(10)
